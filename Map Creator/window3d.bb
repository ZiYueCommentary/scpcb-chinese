...
Graphics3D ResWidth,ResHeight,32,2
...
Repeat
	...
	If (Not IsRemote)
		...
		If PickedRoom<>Null
			...
			Color 0,0,0
			Rect 2,32-3,StringWidth("房间名： "+rname),StringHeight("房间名： "+rname)+3
			Rect 2,52-3,StringWidth("房间X: "+rX),StringHeight("房间X: "+rX)+3
			Rect 2,72-3,StringWidth("房间Z: "+rZ),StringHeight("房间Z: "+rZ)+3
			...
			If PickedRoom\event<>""
				...
				Color 0,0,0
				Rect 2,92-3,StringWidth("房间事件： "+ename),StringHeight("房间事件： "+ename)+3
				Rect 2,112-3,StringWidth("房间事件概率: "+Int(echance*100)+"%"),StringHeight("房间事件概率: "+Int(echance*100)+"%")+3
				
				Color 255,255,255
				Text 2,92,"房间事件: "+ename
				Text 2,112,"房间事件概率: "+Int(echance*100)+"%"
			EndIf
		EndIf
		...
		If CurrSelectedRoom<>Null Then
			...
			Color 0,0,0
			Rect (ResWidth-2)-StringWidth("已选房间: "+rname),2-3,StringWidth("已选房间: "+rname),StringHeight("Selected room: "+rname)+3
			
			Color 255,255,255
			Text (ResWidth-2)-StringWidth("已选房间: "+rname),2,"已选房间: "+rname
		EndIf
	...
	Else
		Flip False
	EndIf
Until api_FindWindow("BlitzMax_Window_Class" , ConvertToANSI("SCP - 收容失效 地图制作器 v"+versionnumber)) = 0
End
