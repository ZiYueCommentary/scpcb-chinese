...
WinHandle=CreateWindow("SCP - 收容失效 地图制作器 v"+versionnumber,GraphicsWidth()/2-ResWidth/2, GraphicsHeight()/2-ResHeight/2,ResWidth,ResHeight,0, 13) 
...
InsertGadgetItem(tab,0,"2D/地图制作器")
InsertGadgetItem(tab,1,"3D/地图查看器")
SetGadgetLayout tab , 3,3,2,2

tab2=CreateTabber(300,5,ResWidth/4+20,ResHeight-100,winhandle)
InsertGadgetItem(tab2,0,"设施")
InsertGadgetItem(tab2,1,"森林")
InsertGadgetItem(tab2,2,"维修通道")
SetGadgetLayout tab2 , 3,3,2,2

SetStatusText(Loadingwindow, "正在启动")
; Now create a whole bunch of menus and sub-items - first of all the FILE menu
file=CreateMenu("文件(&F)",0,menu) ; main menu
CreateMenu "新建",0,file ; child menu 
CreateMenu "打开",1,file ; child menu 
CreateMenu "",1000,file ; Use an empty string to generate separator bars
CreateMenu "保存",2,file ; child menu 
CreateMenu "另存为...",3,file ; child menu 
CreateMenu "",1000,file ; Use an empty string to generate separator bars
CreateMenu "退出",10001,file ; another child menu

options=CreateMenu("设置(&O)",0,menu)
event_default = CreateMenu("默认设置房间事件",15,options)
Global adjdoor_place = CreateMenu("在3D预览中放置门",16,options)
CreateMenu "",1000,options
zone_trans = CreateMenu("地图设置",18,options)
author_descr = CreateMenu("修改作者和描述",19,options)
CreateMenu "",1000,options
...
; Now the Edit menu
edit=CreateMenu("帮助(&H)",0,menu) ; Main menu with Alt Shortcut - Use & to specify the shortcut key
CreateMenu "手册"+Chr$(8)+"F1",6,edit ; Another Child menu with Alt Shortcut
CreateMenu "关于"+Chr$(8)+"F12",40,edit ; Child menu with Alt Shortcut
...
SetStatusText(Loadingwindow, "正在生成2D场景...")
Optionwin=CreateWindow("修改镜头设置", GraphicsWidth()/2-160,GraphicsHeight()/2-120,300,280,winhandle,1)
...
map_settings=CreateWindow("地图设置", GraphicsWidth()/2-120,GraphicsHeight()/2-80,240,160,winhandle,1)
HideGadget map_settings

zonetext = CreateLabel("区域过渡设置：",10,10,200,20,map_settings)
labelzonetrans1 = CreateLabel("轻收容至重收容过渡",20,60,80,25,map_settings)
Global zonetrans1 = CreateTextField(20,40,80,20,map_settings)
SetGadgetText zonetrans1,5
labelzonetrans2 = CreateLabel("重收容至办公区过渡",130,60,80,25,map_settings)
...
resetzonetrans = CreateButton("重置",10,90,100,30,map_settings)
applyzonetrans = CreateButton("应用",120,90,100,30,map_settings)

authordescr_settings=CreateWindow("修改作者和描述", GraphicsWidth()/2-200,GraphicsHeight()/2-80,400,200,winhandle,1)
...
map_author_label = CreateLabel("作者：",140,10,160,20,authordescr_settings)
Global descr_text = CreateTextArea(20,80,350,80,authordescr_settings,1)
descr_label = CreateLabel("描述：",140,60,160,20,authordescr_settings)

SetStatusText(Loadingwindow, "正在运行3D查看器...")
...
Until vwprt <> 0
SetStatusText(Loadingwindow, "正在创建3D场景...")
...
Repeat
	...
	If FileType("CONFIG_TO2D.SI")=1
		...
		If CurrMapGrid%=0
			...
			For rt.RoomTemplates = Each RoomTemplates
				If rt = Map(Grid_SelectedX,Grid_SelectedY)
					...
					For i = 0 To 5
						...
						EndIf
					Next
					SetGadgetText room_desc,"房间描述："+Chr(13)+rt\Description
					Exit
				EndIf
			Next 
			...
			If (Not hasEvent)
				...
				DisableGadget event_prob
				GridGadgetText="房间名："+Map(Grid_SelectedX,Grid_SelectedY)\Name+Chr(13)+"旋转角度："+MapAngle(Grid_SelectedX,Grid_SelectedY)+"°"
			Else
				EnableGadget combobox
				If MapEvent(Grid_SelectedX,Grid_SelectedY)<>"" And MapEvent(Grid_SelectedX,Grid_SelectedY)<>"[无]"
					For ev.event = Each Event
						If ev\name = MapEvent(Grid_SelectedX,Grid_SelectedY)
							SetGadgetText event_desc, "事件描述："+Chr(13)+ev\description
							Exit
						EndIf
					Next
				Else
				...
				EndIf
				If MapEvent(Grid_SelectedX,Grid_SelectedY)<>"" And MapEvent(Grid_SelectedX,Grid_SelectedY)<>"[无]"
					SetGadgetText event_prob_label, "事件概率："+Int(MapEventProb(Grid_SelectedX,Grid_SelectedY)*100)+"%"
					SetSliderValue event_prob,Int(MapEventProb(Grid_SelectedX,Grid_SelectedY)*100)-1
					EnableGadget event_prob
					GridGadgetText="房间名："+Map(Grid_SelectedX,Grid_SelectedY)\Name+Chr(13)+"旋转角度："+MapAngle(Grid_SelectedX,Grid_SelectedY)+"°"+Chr(13)+"事件名："+MapEvent(Grid_SelectedX,Grid_SelectedY)+Chr(13)+"事件概率："+Int(MapEventProb(Grid_SelectedX,Grid_SelectedY)*100)+"%"
				Else
					SetGadgetText event_prob_label, ""
					SetSliderValue event_prob,99
					DisableGadget event_prob
		...
		ElseIf CurrMapGrid%=1
			...
			SetSliderValue event_prob,99
			DisableGadget event_prob
			GridGadgetText="房间名："+ForestPlace(Grid_SelectedX,Grid_SelectedY)\Name+Chr(13)+"旋转角度："+ForestPlaceAngle(Grid_SelectedX,Grid_SelectedY)+"°"
		Else
			For rt.RoomTemplates = Each RoomTemplates
			...
			SetSliderValue event_prob,99
			DisableGadget event_prob
			GridGadgetText="房间名："+MTRoom(Grid_SelectedX,Grid_SelectedY)\Name+Chr(13)+"旋转角度："+MTRoomAngle(Grid_SelectedX,Grid_SelectedY)+"°"
		EndIf
								...
								If MouseHit1
									...
									Else
										...
										If Map(x,y)<>Null
											...
											Else
												EnableGadget combobox
												If MapEvent(x,y)<>"" And MapEvent(x,y)<>"[无]"
													For ev.event = Each Event
														If ev\name = MapEvent(x,y)
															SetGadgetText event_desc, "事件描述："+Chr(13)+ev\description
															Exit
												...
												EndIf
												If MapEvent(x,y)<>"" And MapEvent(x,y)<>"[无]"
													SetGadgetText event_prob_label, "事件概率：100%"
													SetSliderValue event_prob,99
													EnableGadget event_prob
												Else
													SetGadgetText event_prob_label, ""
					...
					Else
						...
						If Grid_SelectedX=x And Grid_SelectedY=y
							If PrevSelectedX<>Grid_SelectedX Or PrevSelectedY<>Grid_SelectedY
								ChangeGridGadget = True
								If MapEvent(x,y)<>"" And MapEvent(x,y)<>"[无]"
									GridGadgetText = "房间名："+Map(x,y)\Name+Chr(13)+"旋转角度："+MapAngle(x,y)+"°"+Chr(13)+"事件名："+MapEvent(x,y)+Chr(13)+"事件概率："+Int(MapEventProb(x,y)*100)+"%"
				...
				For y = 0 To ForestGridSize
					...
					Else
						...
						If Grid_SelectedX=x And Grid_SelectedY=y
							If PrevSelectedX<>Grid_SelectedX Or PrevSelectedY<>Grid_SelectedY
								ChangeGridGadget = True
								GridGadgetText = "房间名："+ForestPlace(x,y)\Name+Chr(13)+"旋转角度："+ForestPlaceAngle(x,y)+"°"
							EndIf
						EndIf
					EndIf
				Next
				...
				For y = 0 To MT_GridSize
					...
					Else
						...
						If Grid_SelectedX=x And Grid_SelectedY=y
							If PrevSelectedX<>Grid_SelectedX Or PrevSelectedY<>Grid_SelectedY
								ChangeGridGadget = True
								GridGadgetText = "房间名："+MTRoom(x,y)\Name+Chr(13)+"旋转角度："+MTRoomAngle(x,y)+"°"
							EndIf
						EndIf
					EndIf
				Next
	...
	EID=EventData() 
		...
		EndIf
		If EID=1 Then
			filename$ = RequestFile("打开地图","*cbmap2;*cbmap,*cbmap2,*cbmap",False,"") 
			If filename<>""
				LoadMap(filename$)
			Else
