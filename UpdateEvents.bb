					...
					If e\EventState3>0 Then
						...
						If e\EventState3 < 170 Then
							If e\EventState3 = 1.0 Then
								...
								MusicCHN = StreamSound_Strict("SFX\Music\"+Music(13)+".ogg",CurrMusicVolume)
						...
						If (CoffinDistance < 4.0) And (hasBatteryFor895) And (Not Wearing714) Then
							...
							If Sanity < -1000 Then
								If WearingNightVision > 1
									DeathMSG = "“一名D级人员佩戴一副夜视仪观察SCP-895，其夜视仪可能经过了SCP-914加工。"
									DeathMSG = DeathMSG + "对象可能通过佩戴夜视仪抵消了部分意识效应。夜视仪已被保存起来，以便下一步研究。”"
								Else
									DeathMSG = "“一名D级人员佩戴夜视仪观察SCP-895，导致死亡。”"
								EndIf
					...
					If e\EventState2 = 1 Then ;in the second room
								...
								For i = 9 To 10
									...
									If dist<6.0 Then 
										If dist<100.0*RoomScale Then
											...
											If KillTimer = 0 Then
												DeathMSG = "除了SCP-106受害者典型的腐烂外观外，尸体还具有以前从未观察到的伤口："
												DeathMSG = DeathMSG + "严重颅骨骨折、三根肋骨骨折、肩部骨折和多处严重撕裂伤。"
						...
						If EntityY(Collider) < -1600*RoomScale Then
							...
							Else ;the player is not at the exit, must've fallen down
								...
								If KillTimer => 0 Then 
									PlaySound_Strict HorrorSFX(8)
									DeathMSG = "除了SCP-106受害者典型的腐烂外观外，对象似乎双腿多处严重骨折。"
								...
								If KillTimer => 0 Then 
									For i = 0 To 2
										If Distance(EntityX(Collider),EntityZ(Collider),EntityX(e\room\Objects[i],True),EntityZ(e\room\Objects[i],True)) < 250.0*RoomScale Then
											...
											CameraShake = 1.0
											Kill()
											DeathMSG = "对象D-9341在[已编辑]被特斯拉电门击毙。"
										EndIf
									Next
								EndIf
			...
			Case "room2test1074"
				...
				If PlayerRoom=e\room Then
					...
					If e\EventState > 0.0 Then
						...
						; 我看不懂，但我大受震撼.jpg
						; 看起来跟SCP-1074有关，如果房间刷出，则弹下面这堆话
						; 我不理解他们为什么要删这个SCP，实现一个SCP很容易吗？
						; ——子悦
						Select Floor(e\EventState) 
							Case 250.0
								...
								MsgTimer = 70*5
							Case 500.0
								Msg = "如果你看到了这个，就请反馈亿下bug"
								MsgTimer = 70*5
							Case 700.0
								Msg = "真的，这个SCP不应该生成" ; shouldn't --> should not why juan why --Mark
								MsgTimer = 70*5
							Case 800.0
								BlinkTimer = -2
								BlurTimer = 800
						...
						If PlayerRoom <> e\room Then
							;e\EventState = 2300.0
							If PlayerRoom\RoomTemplate\Name = "pocketdimension" Then DeathMSG = "对象D-9341被SCP-106捕获并杀死，尸体已被火化。"
							Kill()
							Playable = True
						EndIf
						If KillTimer < 0 Then
							...
							;DebugLog "WHERE IS IT?!"	
								...
								If dist < 0.6 Then
									...
									ElseIf e\EventState3>85*70 And e\EventState3-FPSfactor=<85*70	
										DeathMSG = "对象D-9341的尸体在SCP-012旁边的血泊中发现。"
										DeathMSG = DeathMSG + "对象似乎在因失血过多前在谱中续写了三行。"
										Kill()
									EndIf
								...
								If EntityZ(Collider)>Min(EntityZ(e\room\Objects[7],True),EntityZ(e\room\Objects[8],True)) Then
									If EntityZ(Collider)<Max(EntityZ(e\room\Objects[7],True),EntityZ(e\room\Objects[8],True)) Then
										...
										If KillTimer < 0 And Bloodloss =>100 Then
											DeathMSG = "D级人员项目D-9341的尸体在SCP-035收容室找到。"
											DeathMSG = DeathMSG + "对象眼部周围以及口鼻内的血管严重出血，已送去尸检。"
										EndIf
									EndIf
								EndIf
							EndIf
					...
					Select e\EventState
						...
						Case 3
							...
							If (e\EventState2>70*15) Then
								DeathMSG = "在[已编辑]发现了一具覆盖着耳朵的尸体。对象可能遭到了SCP-1048-A的攻击，并因耳朵而窒息。尸体已被送去尸检。"
								Kill()
								e\EventState = 4
								RemoveEvent(e)
							EndIf
					...
					ElseIf e\EventState3 = 2.0
						...
						For itt.ItemTemplates = Each ItemTemplates
							If IsItemGoodFor1162(itt) And Rand(6)=1
								...
								e\EventState3 = 0.0
								If Injuries > 15
									DeathMSG = "在SCP-1126的收容室找到了一具D级人员的尸体。"
									DeathMSG = DeathMSG + "尸检报告证明他的右肺缺失，这表明他受到了SCP-1162的影响。"
			...
			Case "gatea"
				...
				If PlayerRoom = e\room Then 
					...
					If e\EventState = 0 Then
						...
						CreateConsoleMsg("警告！传送至远离此区域的位置可能会导致错误甚至崩溃。")
